...
debugApi = app.config["MALCAP_API_DEBUG"] == "true"
...
opensearchCreds = (
    malcap_utils.ParseCurlFile(app.config["OPENSEARCH_CREDS_CONFIG_FILE"])
...
@app.route(
    f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/agg",
    defaults={'fieldname': 'event.provider'},
    methods=['GET', 'POST'],
)
@app.route(
    f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/agg/<fieldname>",
    methods=['GET', 'POST'],
)
...
@app.route(
    f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/document",
    defaults={'index': app.config["ARKIME_INDEX_PATTERN"]},
    methods=['GET', 'POST'],
)
@app.route(
    f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/document/<index>",
    methods=['GET', 'POST'],
)
def document(index):
...
@app.route(
    f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/index", methods=['GET']
)
@app.route(
    f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/indexes", methods=['GET']
)
@app.route(
    f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/indices", methods=['GET']
)
def indices():
...
@app.route(
    f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/fields", methods=['GET']
)
def fields():
    """Provide a list of fields Malcap "knows about" merged from Arkime's field table, Malcap's
    OpenSearch template for the sessions indices, and Kibana's field list

    Parameters
    ----------
    request : Request
        template - template name (default is app.config["MALCAP_TEMPLATE"])
    ...
    templateName = args['template'] if 'template' in args else app.config["MALCAP_TEMPLATE"]
    pattern = args['pattern'] if 'pattern' in args else app.config["ARKIME_INDEX_PATTERN"]
    arkimeFields = (templateName == app.config["MALCAP_TEMPLATE"]) and (pattern == app.config["ARKIME_INDEX_PATTERN"])
...
@app.route(f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/", methods=['GET'])
@app.route(
    f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/version", methods=['GET']
)
def version():
    """Provides version information about Malcap and the underlying OpenSearch instance
    ...
    -------
    version
        a string containing the Malcap version (e.g., "5.2.0")
    built
        a string containing the Malcap build timestamp (e.g., "2021-12-22T14:13:26Z")
    sha
        a string containing the last commit sha from the Malcap source repository (e.g., "11540a7")
    opensearch
    ...
    return jsonify(
        version=app.config["MALCAP_VERSION"],
...
@app.route(
    f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/ping", methods=['GET']
)
def ping():
...
@app.route(
    f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/alert", methods=['POST']
)
@app.route(
    f"{('/' + app.config['MALCAP_API_PREFIX']) if app.config['MALCAP_API_PREFIX'] else ''}/event", methods=['POST']
)
def event():
    ...
    in Malcap's default visualizations.

    See Malcap's malcap_api_loopback_monitor.json and malcap_api_loopback_destination.json
    ...
    e.g.:
        {
          "alert": {
            "monitor": {
              "name": "Malcap API Loopback Monitor"
            },
            "trigger": {
              "name": "Malcap API Loopback Trigger",
              "severity": 4
            },
            "period": {
