...
if [[ $(curl "${CURL_CONFIG_PARAMS[@]}" -fs -XGET -H'Content-Type: application/json' "${OPENSEARCH_URL}/_cat/indices/arkime_users_v*" | wc -l) < 1 ]]; then
  ...
  echo "Creating default user..."
	...
	$ARKIME_DIR/bin/arkime_add_user.sh "${MALCAP_USERNAME}" "${MALCAP_USERNAME}" "ignored" --admin --webauthonly --webauth $DB_SSL_FLAG
  ...
  for VIEW_FILE in "$ARKIME_DIR"/etc/views/*.json; do
    ...
    jq ". += {\"user\": \"${MALCAP_USERNAME}\"}" < "${VIEW_FILE}" >"${TEMP_JSON}"
  ...
  curl "${CURL_CONFIG_PARAMS[@]}" -sS --output /dev/null -H'Content-Type: application/json' -XPOST "${OPENSEARCH_URL}/arkime_users/_update/$MALCAP_USERNAME" -d "@$ARKIME_DIR/etc/user_settings.json"
